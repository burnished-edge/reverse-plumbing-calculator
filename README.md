...
This tool is used for tenant improvements where avoiding additional plumbing construction is a priority; for example, if you are designing a restaurant in an existing commercial space, the calculator reveals the maximum number of seats you can legally propose without having to trench the floor to build additional restrooms.
...
* **Translating to Real Space:** It then multiplies that maximum occupant load by the CPC Table 4-1 load factor to give you the maximum allowable square footage or seat count for your selected occupancy type.
...
### Math Breakdown
Just like the forward calculator, the expandable **Math Breakdown** section shows how the existing fixtures were evaluated against the code's fractions and step-functions to arrive at required fixture counts for the values entered into the Occupied Space sections.

![Math Breakdown](docs/images/breakdown.png)

### PDF Export
The Print/Save PDF button expands all math breakdowns, removes interactive UI elements, and optimizes the layout for letter-sized printing.

### JSON Import and Export
Save and load existing building configurations.
* **Export:** Download your current fixture counts and settings as a `.json` file.
* **Import:** Upload a previously saved `.json` file to restore your workspace without relying on a database.

### Copy Share Link
Share your existing building analysis. Clicking the **Copy Link** button encodes your current fixture counts and occupancy settings directly into the URL hash, allowing you to share the state of your reverse calculation with clients, engineers, or plan checkers.
...
1. **Inventory Existing Fixtures:** Begin by inputting the number of existing plumbing fixtures currently built in the space. You must categorize Water Closets, Urinals, and Lavatories by their designated sex (Male, Female, or Unisex/Single-Accommodation).
...
> **⚠️ LADBS ONLY**
> 
> This calculator is configured for projects within the jurisdiction of the **Los Angeles Department of Building and Safety (LADBS)**. 
> 
...
> * **San Francisco** applies its own local amendments to CPC Table 422.1 and enforces unique local overlays (such as the Drink Tap Ordinance) that regulate drinking fountains and bottle fillers, altering overall fixture counts.
